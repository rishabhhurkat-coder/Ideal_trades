...
The auxiliary SQLite file `Data/nifty_expiry_dte.db` now exposes these columns:
...
`Eff DTE` is the per-expiry sequence that is `0` on the expiry date and increments backward through earlier rows.
