...
MARKET_LAST_BUCKET_TIME = "15:30:00"
    ...
    *,
    ...
    source as (
        ...
        where trade_date between ? and ?
          and expiry = ?
          and option_type = ?
          and candle_time between time '09:15:00' and time '15:30:00'
    ),
    aggregated as (
        select
            trade_date,
...
def _duckdb_pipeline_from_frame(
    frame: pd.DataFrame,
    *,
    symbol: str,
    file_expiry: date,
    ...
    if stats_row is None:
        raise RuntimeError("DuckDB stats row missing.")
...
def build_file_day_batches_from_frame(
    frame: pd.DataFrame,
    *,
    source_file: str,
    symbol: str,
    ...
    start = time.perf_counter()
    metrics, data_rows = _duckdb_pipeline_from_frame(
        frame,
        symbol=symbol,
        file_expiry=file_expiry,
        option_type=option_type,
